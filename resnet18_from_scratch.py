...
logging.basicConfig(
    filename=os.path.join(log_dir, f'training_log_resnet_scratch_{time.strftime("%Y%m%d_%H%M%S")}.txt'),
    level=logging.INFO,
...
model.load_state_dict(torch.load('resnet18_from_scratch.pth'))

# 损失函数和优化器（降低学习率）
criterion = nn.BCELoss()
optimizer = optim.Adam(model.parameters(), lr=5e-4)  # 学习率调低，便于微调
...
def train_model(model, criterion, optimizer, num_epochs=20):
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), 'resnet18_from_scratch.pth')
    ...
    plt.savefig('training_curve_resnet18_from_scratch.png')
    plt.show()
